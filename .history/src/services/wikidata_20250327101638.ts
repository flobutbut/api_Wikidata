...
export class WikidataService {
  ...
  private buildQuery(options: QueryOptions): string {
    ...
    return `
      SELECT DISTINCT ?item ?itemLabel ?startDate ?endDate ?location ?locationLabel ?isGlobal
      WHERE {
        ${parentFilter}
        ...
        # Vérifier si c'est une période globale
        BIND(EXISTS {
          ?item wdt:P31 wd:Q12136.  # instance of geological period
          ?item wdt:P276 wd:Q2.      # located in Earth
        } AS ?isGlobal)
        # Vérifier si c'est une période archéologique
        BIND(EXISTS {
          ?item wdt:P31 wd:Q12136.  # instance of geological period
          ?item wdt:P279* wd:Q12136.  # subclass of geological period
          ?item wdt:P31 wd:Q12136.  # instance of archaeological period
        } AS ?isArchaeological)
        FILTER(?itemLabel != "")
      }
  ...
  private transformResponse(bindings: any[]): GeologicalPeriod[] {
    return bindings.map(binding => {
      ...
      console.log('Is Global:', binding.isGlobal?.value);
      console.log('Is Archaeological:', binding.isArchaeological?.value);

      // Gestion de la localisation
      let locationDescription: string | undefined;
      if (binding.isGlobal?.value === 'true') {
        locationDescription = 'Toute la Terre';
      } else if (binding.isArchaeological?.value === 'true') {
        // Pour les périodes archéologiques, on affiche "Toute la Terre" par défaut
        locationDescription = 'Toute la Terre';
      } else if (binding.locationLabel?.value) {
        // Si la localisation est "Earth" ou "Terre", on affiche "Toute la Terre"
        if (binding.locationLabel.value.toLowerCase().includes('terre') || 
            binding.locationLabel.value.toLowerCase().includes('earth')) {
          locationDescription = 'Toute la Terre';
        } else {
          locationDescription = `Localisation: ${binding.locationLabel.value}`;
        }
      } else if (binding.location?.value) {
        // Si on a une localisation mais pas de label, on affiche "Toute la Terre"
        locationDescription = 'Toute la Terre';
      }
