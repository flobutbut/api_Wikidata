...
export class WikidataService {
  ...
  private buildQuery(options: QueryOptions): string {
    ...
    const parentFilter = parentId 
      ...
      : `VALUES ?item { wd:Q12308942 wd:Q12308943 wd:Q392928 wd:Q12308944 }`; // Liste des principales périodes
    
    return `
      SELECT DISTINCT ?item ?itemLabel (YEAR(?startDate) AS ?start) (YEAR(?endDate) AS ?end) ?type ?typeLabel
      WHERE {
        ${parentFilter}
  ...
  private transformResponse(bindings: any[]): GeologicalPeriod[] {
    return bindings.map(binding => {
      ...
      return {
        ...
        description: binding.typeLabel?.value,
