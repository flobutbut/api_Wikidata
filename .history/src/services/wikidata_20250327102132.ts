...
export class WikidataService {
  ...
  private transformResponse(bindings: any[]): GeologicalPeriod[] {
    return bindings.map(binding => {
      ...
      const label = binding.itemLabel.value.toLowerCase();
      
      // Vérifier si c'est une période archéologique ou globale
      if (label.includes('âge du bronze') || 
          label.includes('âge de la pierre') || 
          label.includes('âge du fer') ||
          label.includes('paléolithique') ||
          label.includes('néolithique') ||
          label.includes('mésolithique')) {
