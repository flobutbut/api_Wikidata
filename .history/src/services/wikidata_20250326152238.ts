...
export class WikidataService {
  ...
  private buildQuery(options: QueryOptions): string {
    ...
    // Liste des principales périodes géologiques avec les bons identifiants Q
    const mainPeriods = [
      'wd:Q12308941', // Période géologique (classe)
      'wd:Q12308942', // Éon
      'wd:Q12308943', // Ère
    ...
    const parentFilter = parentId 
      ...
      : `VALUES ?item { ${mainPeriods.join(' ')} }`;
    
    return `
      SELECT DISTINCT ?item ?itemLabel ?description ?startDate ?endDate ?parentPeriod ?parentLabel
      WHERE {
        ${parentFilter}
        ...
        SERVICE wikibase:label { 
          ...
          ?parentPeriod rdfs:label ?parentLabel.
  ...
  private transformResponse(bindings: any[]): GeologicalPeriod[] {
    return bindings.map(binding => {
      ...
      return {
        ...
        childPeriods: [] // Nous chargerons les sous-périodes à la demande
      };
    });
  }
  ...
  async getGeologicalPeriods(options: QueryOptions = {}): Promise<GeologicalPeriod[]> {
    ...
    return this.retryOperation(async () => {
      ...
      try {
        ...
        console.log('Réponse de Wikidata:', response.data);
