...
export class WikidataService {
  ...
  private buildQuery(periodId: string | null = null): string {
    return `
      SELECT DISTINCT ?item ?itemLabel ?startDate ?endDate ?location ?locationLabel WHERE {
        ...
        FILTER(LANG(?itemLabel) = "${this.currentLanguage}").
        ...
        OPTIONAL { 
          ...
          FILTER(LANG(?locationLabel) = "${this.currentLanguage}").
        }
  ...
  async getGeologicalPeriods(options: QueryOptions = {}): Promise<GeologicalPeriod[]> {
    ...
    return this.retryOperation(async () => {
      const query = this.buildQuery(options.parentId);
