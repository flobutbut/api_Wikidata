...
export class WikidataService {
  ...
  private buildQuery(options: QueryOptions): string {
    ...
    // Requête de débogage pour voir toutes les propriétés
    if (parentId === 'Q11761') { // Si on cherche les enfants de l'âge du bronze
      return `
        SELECT DISTINCT ?item ?itemLabel ?startDate ?endDate ?p ?pLabel ?o ?oLabel
        WHERE {
    ...
    return `
      ...
      WHERE {
        ...
        OPTIONAL { ?item wdt:P582 ?endDate. }
        OPTIONAL { 
          ?item wdt:P276 ?location.  # situé dans
          SERVICE wikibase:label { 
            bd:serviceParam wikibase:language "${language},en". 
  ...
  private transformResponse(bindings: any[]): GeologicalPeriod[] {
    // Si on a les propriétés de débogage
    if (bindings[0]?.p) {
      console.log('Propriétés trouvées:');
      bindings.forEach(binding => {
        console.log(`${binding.itemLabel.value}: ${binding.pLabel.value} = ${binding.oLabel.value || binding.o.value}`);
      });
      return []; // On ne retourne rien en mode débogage
    }
